...
c
c Objetivo:     Calcula anomalias respecto a la media
c Metodo:       Obvio
c Comentarios:  
c
...
chelpfin
c
	parameter(nn=60000,nm=12,spval=9E10)
        integer nmes
        real miss
	dimension f(nn),rmed(nm,nn),ia(4),nd(nm,nn)
	character*50 inputfile,outputfile,outputfile2
        rmed=0.
        nd=0
c
        miss=9999.9
...
c
	...
	iy=int(ia(1)/10000)  !ano
	mon=idate-iy*100     !mes
c        if (mon.eq.0) mon=1
c
	do 20 i=1,ia(4)
	i1=1-int(abs(f(i))/spval)       !0 se missing e 1 se no missing
	rmed(mon,i)=rmed(mon,i)+i1*f(i) !suma de valores 
	nd(mon,i)=nd(mon,i)+i1          !contador por meses
 20	continue
c
	goto 10 
...
c
        write(3,*)'---- Mes: ',k,'--------------->'
        write(3,*)k,(rmed(k,i),i=1,10)
        write(3,*)k,(nd(k,i),i=1,10)
        write(3,*)'-------------------------'
        write(30)k,1,1,ia(4)
